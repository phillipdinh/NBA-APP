...
function GameListItem(props) {
    ...
    startTime.setHours((startTime.getUTCHours() >= 7) ? startTime.getUTCHours() - 7 : startTime.getUTCHours() - 7 + 24)
    ...
    return (
        <GameListItemContainer>
            ...
            <ScoresTimeDiv>
                {(game.status.short === 2) ? <ScoreP>{game.scores.visitors.points}</ScoreP> : <></>}
                {(game.status.short === 3) ? <ScoreP>{game.scores.visitors.points}</ScoreP> : <></>}
                <TimePeriodDiv>
                {(game.status.short === 2 && game.periods.current != 0) ? <TimeP>Q{game.periods.current}</TimeP> : <></>}
                    {(game.status.short === 2 && game.status.clock != null) ? <TimeP>{game.status.clock}</TimeP> : <></>}
                    {(game.status.short === 1) ? <TimeP>{startTimeString}</TimeP> : <></>}
                    {(game.status.short === 3) ? <TimeP>Final</TimeP> : <></>}
                </TimePeriodDiv>
                {(game.status.short === 2) ? <ScoreP>{game.scores.home.points}</ScoreP> : <></>}
                {(game.status.short === 3) ? <ScoreP>{game.scores.home.points}</ScoreP> : <></>}
            </ScoresTimeDiv>
        </GameListItemContainer>
    )
}
