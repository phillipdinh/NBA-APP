		...
		const getGames = async () => {
			...
			try {
				...
				let finishedGames = games.filter(
					(game) =>
						game.status.short === 3 && checkIfGameWasToday(game.date.start)
				)
				...
				if (resp.status === 200) {
					...
					notStartedGames = notStartedGames.concat(
						gamesTomorrow.filter(
							(game) =>
								game.status.short === 1 &&
								!checkIfGameWasToday(game.date.start)
						)
					)
					finishedGames = finishedGames.concat(
						gamesTomorrow.filter(
							(game) =>
								game.status.short === 3 &&
								!checkIfGameWasToday(game.date.start)
