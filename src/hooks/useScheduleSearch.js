...
import { setTeamSchedule, setTeamScheduleLoading, setTeamScheduleError } from '../redux/actions'
...
function useScheduleSearch() {
    ...
    useEffect(() => {
        async function getTeamSchedule() {
            ...
            let response
            try {
                response = await api.get(`games?team=${id}&season=2022&league=standard`)
                if (response.status === 200) {
                    console.log(response)
                    const games = response.data.response.map((game) => {
                        const title = game.teams.visitors.id === parseInt(id) ? `@${game.teams.home.name}` : `${game.teams.visitors.name}`
                        return {
                            id: game.id,
                            title: title,
                            start: moment(game.date.start),
                            end: game.date.end != null ? moment(game.date.end) : moment(game.date.start).add(2, 'h')
                        }
                    })
