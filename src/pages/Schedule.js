...
import { getAllGames, getFinishedGames, getGamesLoading, getLiveGames, getNotStartedGames } from "../redux/selectors"
...
function getDesiredGames (desiredDate, game) {
	...
	const gameDatePST = new Date(gameDate.getTime() - gameDate.getTimezoneOffset() * 60000 - 8 * 3600000);
...
function Schedule(props) {
	...
	const day7DateString = day7Date.toISOString().split('T')[0]
	...
	const gamesForTomorrow = allGames.filter((game) => getDesiredGames(tomorrowDate, game))
                        ...
                        { (
							<>
								...
								<GameContainer>
									...
									{(loading) ? <></> : gamesForToday.map((game) => <GameListItem2 key={game.id} game={game}/>)}
									{(gamesForToday.length === 0) ? <EmptyMessage>No games</EmptyMessage> : <></>}
								</GameContainer>

								<GameContainer>
									<h3>{tomorrowDateToDisplay} (Tomorrow)</h3>
									{(loading) ? <></> : gamesForTomorrow.map((game) => <GameListItem2 key={game.id} game={game}/>)}
									{(gamesForTomorrow.length === 0) ? <EmptyMessage>No games</EmptyMessage> : <></>}
								</GameContainer>

								<GameContainer>
									<h3>{day3DateToDisplay}</h3>
									{(loading) ? <></> : gamesForDay3.map((game) => <GameListItem2 key={game.id} game={game}/>)}
									{(gamesForDay3.length === 0) ? <EmptyMessage>No games</EmptyMessage> : <></>}
								</GameContainer>

								<GameContainer>
									<h3>{day4DateToDisplay}</h3>
									{(loading) ? <></> : gamesForDay4.map((game) => <GameListItem2 key={game.id} game={game}/>)}
									{(gamesForDay4.length === 0) ? <EmptyMessage>No games</EmptyMessage> : <></>}
								</GameContainer>

								<GameContainer>
									<h3>{day5DateToDisplay}</h3>
									{(loading) ? <></> : gamesForDay5.map((game) => <GameListItem2 key={game.id} game={game}/>)}
									{(gamesForDay5.length === 0) ? <EmptyMessage>No games</EmptyMessage> : <></>}
								</GameContainer>

								<GameContainer>
									<h3>{day6DateToDisplay}</h3>
									{(loading) ? <></> : gamesForDay6.map((game) => <GameListItem2 key={game.id} game={game}/>)}
									{(gamesForDay6.length === 0) ? <EmptyMessage>No games</EmptyMessage> : <></>}
								</GameContainer>

								<GameContainer>
									<h3>{day7DateToDisplay}</h3>
									{(loading) ? <></> : gamesForDay7.map((game) => <GameListItem2 key={game.id} game={game}/>)}
									{(gamesForDay7.length === 0) ? <EmptyMessage>No games</EmptyMessage> : <></>}
								</GameContainer>
