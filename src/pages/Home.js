		...
		<div>
			...
			<LiveGamesContainer>
				{loading ? (
					...
					liveGames.map((game) => <GameListItem key={game.id} game={game} />)
				)}
				{liveGames.length === 0 ? (
					<EmptyMessage>No live games</EmptyMessage>
				) : (
					<></>
				)}
			</LiveGamesContainer>
			<FinishedGamesContainer>
				...
				{finishedGames.length === 0 ? (
					<EmptyMessage>No finished games</EmptyMessage>
				) : (
					<></>
				)}
			</FinishedGamesContainer>
			<NotStartedGamesContainer>
				...
				{notStartedGames.length === 0 ? (
					<EmptyMessage>No games that haven't started yet</EmptyMessage>
				) : (
					<></>
				)}
			</NotStartedGamesContainer>
