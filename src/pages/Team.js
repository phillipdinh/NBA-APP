...
import { getLoadingTeamPlayers, getTeamPlayersError, getTeamPlayers, getTeams, getTeamsError, getLoadingTeams, getTeamSchedulesError, getLoadingTeamSchedules } from '../redux/selectors'
...
function Team(props) {
    ...
    return <>{ (loadingTeams || loadingTeamPlayers || scheduleLoading) ? <Spinner/> : <TeamContainer>
            <TeamName>
                {teamsError != "" ? <Notice message={teamsError} error={true}/> : (team.name || "")}
            </TeamName>
            <Divider>
                <PlayersContainer>
                    <PlayersTitle>Roster</PlayersTitle>
                    { teamPlayersError != "" ? <Notice message={teamPlayersError} error={true}/> : <PlayersList>
                            {(teamPlayers[id] || []).map((player) => {if (hasPlayerData(player)) return <PlayerListItem key={player.id} player={player}/>})}
                        </PlayersList>
                    }
                </PlayersContainer>
                <ScheduleContainer>
                    { scheduleError != "" ? <Notice message={scheduleError} error={true}/> : <div>
                        <Calendar
                        localizer={localizer}
                        events={schedule}
                        startAccessor="start"
...
function hasPlayerData(player) {
    return player.leagues.standard.jersey && player.leagues.standard.pos && player.firstname && player.lastname && player.leagues.standard.active
}
